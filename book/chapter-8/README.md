...
ตามหลักการพื้นฐานของสถาปัตยกรรม Flux นั้น store จะเปลี่ยนสถานะเพื่อตอบสนองต่อ actions ใน method `update` เราจะส่ง `action` แต่เรายังสามารถส่งฟังก์ชัน `change` การเรียกฟังก์ชันนี้เพื่อเรียก consumer:
...
*ถ้าหากว่าเรา push `change` กับ `store` ในอาเรย์ `_stores` หลังจากนั้นให้ `dispatch` เรียก method `update` และส่ง `action` กับ `change` ไปในฟังก์ชัน*
